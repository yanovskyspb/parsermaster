...
require 'links_functions.php';
	...
	while ($counter < $sites) {
	$sql1 = "SELECT * FROM `!sites` where status = '1' and id = '7' ORDER BY updated ASC LIMIT 1";
	//echo $sql1 . '<br>';
	...
	$num_rows = mysql_num_rows($result1);
		...
		while ($row1 = mysql_fetch_assoc($result1)) {
				...
				$result = mysql_query($sql);
				//echo $links_table;
				include_once(''.$links_table.'.php');

				if (isset($itemsmode)) {
					$sql1 = "SELECT link FROM $links_table WHERE parsed != '1' and linktype = '1' ORDER BY prior DESC LIMIT 3";
					$result1 = mysql_query($sql1) or die(mysql_error());
					$num_rows = mysql_num_rows($result1);
				}
				elseif (isset($catsmode)) {
					$sql1 = "SELECT link FROM $links_table WHERE parsed != '1' and linktype = '2' ORDER BY prior DESC LIMIT 3";
					$result1 = mysql_query($sql1) or die(mysql_error());
					$num_rows = mysql_num_rows($result1);
				}
				else {
					$sql1 = "SELECT link FROM $links_table WHERE parsed != '1' ORDER BY prior DESC LIMIT 3";
					$result1 = mysql_query($sql1) or die(mysql_error());
					$num_rows = mysql_num_rows($result1);
				}
				if ($num_rows != 0) {
