...
foreach ($files as $file) {
	...
	if ($file != '.' && $file != '..') {
		...
		$newdir = 100000 + $d;
		$newpath = 'temp/links_jossandmain/' . $newdir . '/' . $file;
...
foreach ($newfiles as $new) {
	...
	$new_dir = $new['newdir'];
	$link_id = $new['link_id'];
	$sql = "UPDATE links_jossandmain SET local_link = '$new_path', timestring = now() WHERE id = '$link_id'";
	//echo $sql . ';<br>';
	$result = mysql_query($sql);
	$dir_name = 'temp/links_jossandmain/' . $new_dir;
